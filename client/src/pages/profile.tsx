...
const Profile = () => {
  ...
  // Fetch already saved profile details when the component mounts
  useEffect(() => {
          ...
          .then(data => {
            if (data.profile) {
              setFormData(prev => ({
                ...
                skills: {
                  technical: data.profile.skills?.technical || prev.skills.technical,
                  soft: data.profile.skills?.soft || prev.skills.soft,
                  languages: data.profile.skills?.languages || prev.skills.languages,
                }
              }));
              // If a profile picture path is returned, build its URL for preview.
              if (data.profile.personalInfo?.profilePicture) {
                setPreviewUrl(`http://localhost:5000/${data.profile.personalInfo.profilePicture}`);
              }
            }
          })
  ...
  const handleResumeUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Process resume for autofill
      const autofillFormData = new FormData();
      autofillFormData.append('resume', file);

      const autofillResponse = await fetch('http://localhost:5001/api/auto-fill-resume', {
        method: 'POST',
        body: autofillFormData,
      });
  ...
  const handleProfilePictureChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Store the file in a separate state variable
    setProfilePictureFile(file);
    
    // Optionally, update the email or other fields if needed but don't overwrite the file with a string
    // handleInputChange('personalInfo', 'profilePicture', file); <-- Remove or comment out this line
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (profilePictureFile) {
        console.log("Appending profile picture file:", profilePictureFile);
        submitData.append(
          'profilePicture', 
          profilePictureFile,
          'profile.jpg'
